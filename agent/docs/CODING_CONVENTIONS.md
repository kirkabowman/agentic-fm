...
> **Migration note**
>
> The A2X solution is currently being migrated to the conventions in this document. Older code in the solution may still show the previous patterns — most notably `~camelCase` for Let-block variables (now `_camelCase`) and `camelCase` for fields (now `TitleCase`). Do not "fix" pre-existing code in passing — only follow the new conventions for **new** code or when the developer explicitly asks for a migration pass on a specific script or field.
...
| Calculation-scoped (Let) | `_`                        | `_lineTotal`       |
| Private global           | `$$~` (ALL_CAPS with dots) | `$$~PRIVATE.CACHE` |

- Use camelCase after the prefix for all variables
- Boolean variables use present-tense verbs or adjectives: `$hasReturns`, `_isTrailing`, `$isValid`
...
| ------------- | ---------------------- | -------------------------------- |
| Regular       | TitleCase              | `FirstName`, `TotalAmount`       |
| Summary       | TitleCase, descriptive | `SummaryCountInvoices`           |
| Global        | ALL_CAPS               | `GLOBALFIELD`                    |
| Primary key   | `ID`                   | `ID`                             |
| Foreign key   | `ID_EntityName`        | `ID_Customer`, `ID_IntakeAILine` |
...
```
Let ( [
		_subtotal = Invoice_LineItems::UnitPrice * Invoice_LineItems::Quantity ;
		_discount = _subtotal * $discountRate
	];
...
- Spaces before and after comparison and assignment operators: `$foo = bar`, `_total > 0`
...
- Boolean variable names use present-tense verbs or adjectives: `$isComplete`, `_hasValue`
...
Set Field By Name [ GetFieldName ( Invoice::Status ) ; "Sent" ]
GetField ( GetFieldName ( Invoice::Status ) )

# Brittle — breaks silently on field rename
Set Field By Name [ "Invoice::Status" ; "Sent" ]
GetField ( "Invoice::Status" )
```
